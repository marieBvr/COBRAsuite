...
variations_to_process=variations_col.find({"mapping_file":{"$exists":False}})

logger.info("Found %d variations tables to process",variations_to_process.count())

for map_doc in variations_to_process:
	...
	try:
        variations_col.update({"_id":map_doc['_id']},{"$set":{"mapping_file":sheet_values}})
		#break
		...
		variations_col.update({"_id":map_doc['_id']},{"$set":{"mapping_file":{"species":species,"file":fp.data_file}}})
		# Close opened file
		file.close()

logger.info("Indexation on field \"mapping_file.Gene ID\" from collection \"variations\"")
variations_col.create_index("mapping_file.Gene ID",sparse=True)